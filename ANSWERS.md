...
- **ad_id**: must be a primary key, that's why I test that it is unique and not null with a severity error (maybe a same ad could also advertise more than 1 product? Maybe, but accordingly to what I see that's not the case, that's way I feel confident in considering it as primary key)
...
- **order_id**: My hypothesis is that one order can contain only one product because that's the evidence with the data at my disposal, if an unique order can be associated with more products that would change something (e.g. primary key).
- **order_time**: only difference is that we cast order_time to two columns, order_date and order_time, since the final marts model is aggregated by day, however we keep time for possible models in the future (e.g. analysis on intra day seasonality of sales)
...
- **product_marketing_category**: it can of course be not unique, if it's null I wanna raise a warning so that we can update our catalogue but I don't wanna stop the pipeline (it will be coalesced to 'Unknown Category' if that happens)
...
- **Split the models per diffrent datasets**: probably bulding a macros for definfing a custom_schema if we needed to deploy everything in BigQuery for examples. As of now every model and table land in the same dataset
